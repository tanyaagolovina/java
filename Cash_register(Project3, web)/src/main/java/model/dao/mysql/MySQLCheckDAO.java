...
public class MySQLCheckDAO extends CheckDAO {
    ...
    public Check cancelProductInCheck(int checkID, int productID) {
        ...
        try (PreparedStatement cancelProduct = connection.prepareStatement(queriesManager.getProperty("check.DELETE_PRODUCT"))) {
            ...
            productDAO.returnToStock(productID, getAmountOfProduct(checkID, productID));
    ...
    private List<Product> getAllProductsInRange(HashMap<Integer, Integer> productIdAndAmount) {
        ...
        MySQLProductDAO productDAO = new MySQLProductDAO();
        List amount = new ArrayList<>(productIdAndAmount.values());
        List keys = new ArrayList<>(productIdAndAmount.keySet());
    ...
    private HashMap<Integer, Integer> extractProductIdAndAmount(ResultSet result) throws SQLException {
    ...
    private int getAmountOfProduct(int checkID, int productID) throws SQLException {
        int amount = 0;
