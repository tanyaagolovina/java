...
public class MySQLProductDAO extends ProductDAO {
    ...
    //private final String ALL_PRODUCTS_FROM_RANGE = "SELECT name,price FROM goods WHERE productID = ?";
    ...
    public Product getByID(int ID){
        ...
        try(PreparedStatement ps = connection.prepareStatement(GET_BY_ID)){
            ...
            product = parseResultSet(result).get(0);
        } catch(SQLException e){
            e.printStackTrace();
        }
        return product;
    ...
    public boolean isInStock(int productID, int amount){
        int amountInStock = 0;
        try(PreparedStatement ps = connection.prepareStatement(GET_AMOUNT_OF_PRODUCT)) {
    ...
    public boolean subtractFromStock(int productID, int amount) throws SQLException{
        boolean flag;
        try(PreparedStatement ps = connection.prepareStatement(UPDATE_AMOUNT)){
